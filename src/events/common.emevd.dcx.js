...
$Event(0, Default, function() {
    ...
    $InitializeEvent(0, 5750025, 80000, 7600, 1055420610, 1055420660);
    $InitializeEvent(1, 5750025, 80100, 7610, 1055420620, 1055420661);
    $InitializeEvent(2, 5750025, 81100, 7640, 1055420630, 1055420662);
    $InitializeEvent(3, 5750025, 80200, 7620, 1055420640, 1055420663);
    $InitializeEvent(4, 5750025, 80300, 7630, 1055420650, 1055420664);
    $InitializeEvent(0, 5750026);
...
$Event(5750020, Restart, function() {
    ...
    // Black conversion has its own encounter listener, independent of full-set effects.
...
// SACRED Black Scaled Set: host-owned qualification and pending inventory snapshot.
// 1055420600 = pending; 0610..0659 = five 10-bit counts; 0660..0664 = worker completion.
$Event(5750021, Restart, function() {
    DisableNetworkSync();
    EndIf(!PlayerIsInOwnWorld());
    if (!EventFlag(1055420600)) {
        EndIf(EventFlag(16000800));
        outcome = CharacterHPValue(10000) <= 0
            || (EventFlag(16002802) && CharacterHPValue(16000800) <= 0);
        WaitFor(EventFlag(16000800) || (PlayerInMap(16, 0, 0, 0)
            && EventFlag(16002801) && outcome));
        // Vanilla sets its defeat flag after the death delay; missed saves cannot qualify later.
        EndIf(EventFlag(16000800));
        // Sample equipment at the outcome, never wait for equipment after the boss has died.
        EndIf(!(PlayerHasArmorEquipped(ArmorType.Head, 80000, -1)
            || PlayerHasArmorEquipped(ArmorType.Body, 80100, -1)
            || PlayerHasArmorEquipped(ArmorType.Body, 81100, -1)
            || PlayerHasArmorEquipped(ArmorType.Arms, 80200, -1)
            || PlayerHasArmorEquipped(ArmorType.Legs, 80300, -1)));
        StoreItemAmountHeldInEventValue(ItemType.Armor, 80000, 1055420610, 10);
        StoreItemAmountHeldInEventValue(ItemType.Armor, 80100, 1055420620, 10);
        StoreItemAmountHeldInEventValue(ItemType.Armor, 81100, 1055420630, 10);
        StoreItemAmountHeldInEventValue(ItemType.Armor, 80200, 1055420640, 10);
        StoreItemAmountHeldInEventValue(ItemType.Armor, 80300, 1055420650, 10);
        BatchSetEventFlags(1055420660, 1055420664, OFF);
        SetEventFlagID(1055420600, ON);
        SaveRequest();
    }
    WaitFor(AllBatchEventFlags(1055420660, 1055420664));
    SetEventFlagID(1055420600, OFF);
    SaveRequest();
    EndEvent();
});

// One-for-one carried-item conversion. Never use storage-inclusive possession checks.
$Event(5750025, Restart, function(armorId, itemLotId, countFlag, doneFlag) {
    DisableNetworkSync();
    EndIf(!PlayerIsInOwnWorld());
    WaitFor(EventFlag(1055420600));
    GotoIf(L1, EventFlag(doneFlag));
    WaitFor(CharacterHPValue(10000) > 0);
    WaitFixedTimeSeconds(1);
L0:
    GotoIf(L1, EventValue(countFlag, 10) == 0 || !PlayerHasItem(ItemType.Armor, armorId));
    WaitFor(CharacterHPValue(10000) > 0);
    // No yield between debit, credit and the saved remaining-count update.
    RemoveItemFromPlayer(ItemType.Armor, armorId, 1);
    AwardItemLot(itemLotId);
    EventValueOperation(countFlag, 10, 1, 0, 1, CalculationType.Sub);
    WaitFixedTimeFrames(1);
    Goto(L0);
L1:
    ClearEventValue(countFlag, 10);
    SetEventFlagID(doneFlag, ON);
    WaitFor(!EventFlag(1055420600));
    RestartEvent();
});

// SACRED Hewg forging presentation. Dialogue owns the request; the item lot owns collection.
// Restore picture/control on interrupted loads without changing Hewg's story flags.
$Event(5750026, Restart, function() {
    DisableNetworkSync();
    EndIf(!PlayerIsInOwnWorld());
    if (EventFlag(1055420701) || EventFlag(1055420702)) {
        FadeToBlack(0, 0, false, 0);
    }
    SetEventFlagID(1055420701, OFF);
    SetEventFlagID(1055420702, OFF);
L0:
    WaitFor(EventFlag(1055420701));
    SetEventFlagID(1055420702, OFF);
    GotoIf(L1, !PlayerInMap(11, 10, 0, 0) || CharacterHPValue(10000) <= 0
        || EventFlag(1055420700) || !EventFlag(9114) || !EventFlag(32080800)
        || !EventFlag(1041500800) || !EventFlag(1036540800));
    FadeToBlack(1, 1, true, 1);
    WaitFor(ElapsedSeconds(1) || !PlayerInMap(11, 10, 0, 0) || CharacterHPValue(10000) <= 0);
    GotoIf(L1, !PlayerInMap(11, 10, 0, 0) || CharacterHPValue(10000) <= 0);
    // Reuse the native smithing idle and its audio (vanilla initialization 11103700).
    ForceAnimationPlayback(11100715, 930010, false, false, false);
    WaitFor(ElapsedSeconds(2) || !PlayerInMap(11, 10, 0, 0) || CharacterHPValue(10000) <= 0);
    GotoIf(L1, !PlayerInMap(11, 10, 0, 0) || CharacterHPValue(10000) <= 0);
    FadeToBlack(0, 1, false, 1);
    WaitFor(ElapsedSeconds(1) || !PlayerInMap(11, 10, 0, 0) || CharacterHPValue(10000) <= 0);
    GotoIf(L1, !PlayerInMap(11, 10, 0, 0) || CharacterHPValue(10000) <= 0);
    SetEventFlagID(1055420702, ON);
L1:
    FadeToBlack(0, 0, false, 0);
    SetEventFlagID(1055420701, OFF);
    Goto(L0);
...
$Event(5750100, Default, function() {
    EndIf(!PlayerIsInOwnWorld());
    WaitFixedTimeSeconds(0.1);
    WaitFor(EventFlag(181) || !EventFlag(50) || EventFlag(1055420918));
    if (EventFlag(1055420918) && !EventFlag(1055420001) && !CharacterHasSpEffect(10000, 1626910)) {
        WaitFor(!CharacterHasSpEffect(10000, 100690) && !CharacterHasSpEffect(10000, 9621))
        if (EventFlag(1055420001) || CharacterHasSpEffect(10000, 1626910)) {
            RestartEvent();
        }
...
// SACRED crystal hardcore: persistent unlock, temporarily suppressed by following Nemesis.
$Event(5750101, Restart, function() {
    EndIf(!PlayerIsInOwnWorld());
    WaitFor(EventFlag(1055420918));
    if (EventFlag(1055420001) || CharacterHasSpEffect(10000, 1626910)) {
        SetEventFlagID(1055420003, OFF);
        SetEventFlagID(1055420010, OFF);
        WaitFor(!EventFlag(1055420001) && !CharacterHasSpEffect(10000, 1626910));
    } else {
        ...
        SetEventFlagID(1055420010, ON);
        WaitFor(EventFlag(1055420001) || CharacterHasSpEffect(10000, 1626910)
            || !EventFlag(1055420010) || !EventFlag(1055420003));
    }
    WaitFixedTimeSeconds(0.1);
    RestartEvent();
});
...
$Event(5750103, Restart, function() {
    EndIf(!PlayerIsInOwnWorld());
    WaitFor(EventFlag(181) || !EventFlag(50) || EventFlag(1055420915));
    if (!EventFlag(1055420001)) {
...
$Event(5750115, Restart, function() {
    EndIf(!PlayerIsInOwnWorld());
    // Stay alive while ineligible; resume without a map reload when the oath ends.
    WaitFor((EventFlag(181) || !EventFlag(50)) && !EventFlag(1055420001)
        && !EventFlag(1055420003) && !EventFlag(1055420000) && !EventFlag(1055420918));
    SuppressSE(SoundType.BGM, 0, false);
    SetEventFlagID(1055420010, OFF);
    ClearSpEffect(10000, 1626975);
    ClearSpEffect(10000, 1626999);
    if (EventFlag(187)) {
        ...
        // Testing only: WaitRandomTimeSeconds(6, 6);
    }
    else if (EventFlag(186)) {
        ...
        WaitRandomTimeSeconds(60, 6300);
        // Testing only: WaitRandomTimeSeconds(6, 6);
    }
    else if (EventFlag(183)) {
        WaitRandomTimeSeconds(60, 7800);
        // Testing only: WaitRandomTimeSeconds(6, 6);
    }
    else if (EventFlag(182)) {
        WaitRandomTimeSeconds(60, 9300);
        // Testing only: WaitRandomTimeSeconds(6, 6);
    }
    else if (EventFlag(181)) {
        WaitRandomTimeSeconds(60, 10800);
        // Testing only: WaitRandomTimeSeconds(6, 6);
    }
    else if (EventFlag(180)) {
        EndEvent();
    }
    if (EventFlag(1055420918) || EventFlag(1055420001) || EventFlag(1055420003) || CharacterDead(10000)
    || CharacterHasSpEffect(10000, 100690) || CharacterHasSpEffect(10000, 9621)) {
        WaitFixedTimeSeconds(10);
        RestartEvent();
    }
    ...
    if (!EventFlag(1055420000) && !EventFlag(1055420010)) {
        ...
        if (!EventFlag(1055420010) || EventFlag(1055420000) || EventFlag(1055420918)) {
            RestartEvent();
        }
        if (CharacterHasSpEffect(10000, 4270) || CharacterHasSpEffect(10000, 4271) || CharacterHasSpEffect(10000, 4272) 
        || CharacterHasSpEffect(10000, 4282) || CharacterHasSpEffect(10000, 4286) || CharacterHasSpEffect(10000, 4288)) {
...
$Event(5750116, Default, function() {
    ...
    // Preserve an earned cycle payout while protecting a newly changed role.
    if (!EventFlag(1055420918) && !EventFlag(1055420001) && !CharacterHasSpEffect(10000, 1626910)) {
        SetEventFlagID(1055420010, OFF);
        SetSpEffect(10000, 1626996);
        SetEventFlagID(1055420000, ON);
